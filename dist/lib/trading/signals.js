import { detectRecentCross, last, macd as macdFn, rsi as rsiFn, sma, } from "./indicators";
...
export function analyzeTicker(def, raw) {
    ...
    const trend = sma50Val && sma200Val
    ...
    if (sma50arr.length && sma200arr.length) {
        ...
        if (cross) {
            signals.push({
                ...
                label: SIGNAL_LABELS[cross.direction === "up" ? "golden-cross" : "death-cross"],
    ...
    if (m.macd.length && m.signal.length) {
        ...
        if (cross) {
            signals.push({
                ...
                label: SIGNAL_LABELS[cross.direction === "up" ? "macd-bull-cross" : "macd-bear-cross"],
                daysAgo: cross.daysAgo,
            });
        }
    }
