 ...
 */
export const REPORT_LOCALE = process.env.REPORT_LOCALE === "en" ? "en" : "zh";
...
function loadAndValidate() {
    ...
    for (let i = 0; i < parsed.length; i++) {
        ...
        if (typeof s.id !== "string" || !s.id)
            throw new Error(`${at}: missing string 'id'`);
        if (seenIds.has(s.id))
            throw new Error(`${at}: duplicate id '${s.id}'`);
        seenIds.add(s.id);
        if (typeof s.name !== "string")
            throw new Error(`${at} (${s.id}): missing 'name'`);
        if (typeof s.url !== "string")
            throw new Error(`${at} (${s.id}): missing 'url'`);
...
function filterByLocale(all) {
    return all.filter((s) => {
        const locales = s.locales ?? ["zh", "en"];
        return locales.includes(REPORT_LOCALE);
    });
}
export const sources = filterByLocale(loadAndValidate());
/**
