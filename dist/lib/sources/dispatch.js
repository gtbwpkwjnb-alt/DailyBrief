...
export async function fetchSource(source) {
    if (source.id === "hackernews")
        return fetchHackerNews(source.id);
    if (source.id === "github-trending")
        return fetchGithubTrending(source.id);
    if (source.id === "v2ex-hot")
        return fetchV2ex(source.id);
    if (source.id === "linuxdo")
        return fetchLinuxDo(source.id);
    if (source.id === "attentionvc-ai")
        return fetchAttentionVc(source.id);
    if (source.id === "huggingface-papers")
        return fetchHuggingfacePapers(source.id, source.keywords);
    if (source.id === "weibo-hot-search")
        return fetchWeiboHot(source.id);
    if (source.id === "baidu-hot-search")
        return fetchBaiduHot(source.id);
    if (source.id === "kr-36-hotlist")
        return fetchKr36Hot(source.id);
    if (source.id === "bilibili-ranking")
        return fetchBilibiliRanking(source.id);
