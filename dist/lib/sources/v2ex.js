 ...
 */
export const V2EX_OFF_TOPIC_RE = /(足浴|按摩|捏\s*jio|相亲|对象|男友|女友|分手|婆|岳|家暴|出轨|彩礼|9\.9\s*元|抽奖|薅羊毛|代理\s*IP|住宅\s*IP|跨境\s*(卖家|IP|电商)|辣椒\s*HTTP|买房|买车|装修|房贷|养老|退休|结婚|生娃|带娃|养娃|减肥|健身|租房|搬家|签证|移民|岛主|离职|裸辞|老赖|存款|新人报道|无聊|发小|废了|工资|加班吐槽|找工作|失业|找对象)/i;
const HEADERS = {
...
async function fetchNode(node) {
    try {
        const r = await fetch(`https://www.v2ex.com/api/topics/show.json?node_name=${node}`, {
            headers: HEADERS,
...
export async function fetchV2ex(sourceId, limit = 25) {
    ...
    for (const list of lists) {
        for (const t of list) {
            ...
            if (seen.has(t.url))
                continue;
            if (V2EX_OFF_TOPIC_RE.test(t.title))
                continue;
