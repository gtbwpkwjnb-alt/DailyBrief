...
export async function generateTradingCommentary(input) {
    ...
    const contextLines = [];
    if (cryptoFearGreed) {
        const classification = REPORT_LOCALE === "en"
            ? cryptoFearGreed.classification
            : cryptoFearGreed.classificationCn;
        const label = REPORT_LOCALE === "en"
    ...
    const RETRY_HINT = REPORT_LOCALE === "en"
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        ...
        try {
            ...
            if (attempt < MAX_ATTEMPTS) {
                console.warn(`[trading-commentary] attempt ${attempt}/${MAX_ATTEMPTS} failed, retrying: ${msg}`);
            }
            else {
                console.warn(`[trading-commentary] all ${MAX_ATTEMPTS} attempts failed: ${msg}`);
            }
        }
    }
    return fallback;
}
async function callOnce(userPrompt, fallback) {
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                console.warn(`[trading-commentary] both JSON.parse and jsonrepair failed; raw at logs/trading-raw-${ts}.txt`);
            }
            catch {
                // best-effort
            }
    ...
    const malformed = picks.find((p) => !p ||
        typeof p !== "object" ||
        typeof p.symbol !== "string" ||
        typeof p.stance !== "string" ||
        typeof p.rationale !== "string" ||
        p.rationale.length < 20);
    if (malformed !== undefined) {
        throw new Error(`watchlist pick has invalid shape: ${JSON.stringify(malformed).slice(0, 120)}`);
    }
    return {
