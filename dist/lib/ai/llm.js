...
import { PRESETS as ANTHROPIC_PRESETS, anthropicCompatModel, runAnthropicCompat, } from "./backends/anthropic-compat";
import { PRESETS as OPENAI_PRESETS, openaiCompatModel, runOpenAICompat, } from "./backends/openai-compat";
...
export function getBackend() {
    ...
    if (!VALID_BACKENDS.has(raw)) {
        throw new Error(`Unknown LLM_BACKEND='${raw}'. Valid values: ${[...VALID_BACKENDS].join(", ")}`);
    }
    return raw;
}
/**
 ...
 */
function getActiveModel(backend) {
    switch (backend) {
        case "claude-cli":
            return CLAUDE_MODEL;
        case "anthropic":
 ...
 */
export function validateBackendCredentials() {
    const backend = getBackend();
    ...
    if (process.env[requiredVar] || process.env.LLM_API_KEY)
        return;
    ...
    if (otherKeysSet.length > 0) {
        lines.push("", "Other API keys ARE present in the environment — likely you meant to use one of those:");
        for (const k of otherKeysSet) {
            lines.push(`  • ${k.var} is set → switch to LLM_BACKEND=${k.backend}`);
        }
        lines.push("", "Fix one of:", `  (a) set LLM_BACKEND to match the key you actually have, or`, `  (b) add ${requiredVar} for the backend you currently selected.`);
    }
    else {
        lines.push("", `Fix: set ${requiredVar} (or the generic LLM_API_KEY).`);
    }
    lines.push("", "Where to set it:", "  • Local:          .env.local at the repo root", "  • GitHub Actions: Settings → Secrets and variables → Actions", "                    (Secrets tab for the API key, Variables tab for LLM_BACKEND)");
